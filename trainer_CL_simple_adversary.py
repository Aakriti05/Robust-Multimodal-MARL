...
def parse_args():
    ...
    parser.add_argument('--max-train-steps', type=int, default=5000_000, help='maximum episode length')
    parser.add_argument('--max-episodes', type=int, default=500_000, help='number of episodes')
...
def evaluate(env, agents, num_runs, args, display=False, max_benchmark_iters=None):
    ...
    for curr_eps in range(num_runs):
        ...
        while not (done or terminal):
            ...
            #if all(agent_dones) and len(agent_done_step_list) < curr_eps + 1:
    ...
    print(np.mean(rewards_all, axis=0), np.std(rewards_all, axis=0))
...
def log_writer(args, agents):
    if args.state_flag: experiment_name = "stateCL"
    elif args.rew_flag: experiment_name = "rewCL"
    elif args.staterew_flag: experiment_name = "staterewCL"
...
def train(args):
    ...
    if args.resume:
        ...
        load_agent_states(args.resume, agents, load_models=args.use_agent_models, checkpoint=70000)
    ...
    if args.evaluate:
        ...
        for key in episode_end_dict:
            print(key, np.mean(episode_end_dict[key]), np.std(episode_end_dict[key]))
    ...
    while (episode_count <= args.max_episodes) and not terminated:
        ...
        if episode_count % args.save_every == 0:
            ...
            with open(files['success_rate_file'], 'a') as f:
                #line = '{},{},{},{}\n'.format(episode_count, sr_mean, env.noise_state, env.noise_rew)
            ...
            # For simple_spread
            if args.rew_flag:
                #if sr_mean['agent_sr_mean'] > 0.9 and sr_mean['adv_sr_mean'] > 0.9: env.noise_rew = env.noise_rew + 1
                if sr_mean['agent_sr_mean'] > 0.9: env.noise_rew = env.noise_rew + 1
            elif args.state_flag:
                #if sr_mean['agent_sr_mean'] > 0.9 and sr_mean['adv_sr_mean'] > 0.9: env.noise_state = env.noise_state + 0.05
                if sr_mean['agent_sr_mean'] > 0.9: env.noise_state = env.noise_state + 0.05
            elif args.action_flag:
                if sr_mean['agent_sr_mean'] > 0.9: env.noise_action = env.noise_action + 0.1

            # For simple_adversary
            # if sr_mean > 0.8:
            #     env.noise_std = env.noise_std + 1
