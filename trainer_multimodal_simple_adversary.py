...
def parse_args():
    ...
    parser.add_argument('--max-train-steps', type=int, default=1000_000, help='maximum episode length')
    parser.add_argument('--max-episodes', type=int, default=1000_000, help='number of episodes')
    ...
    parser.add_argument('--exp-folder-name', type=str, default='default', help='name of experiment')
    ...
    parser.add_argument('--rew-action-flag', default=False, action='store_true')
    parser.add_argument('--rew-state-flag', default=False, action='store_true')
    parser.add_argument('--state-action-flag', default=False, action='store_true')
...
def evaluate(env, agents, num_runs, args, display=False, max_benchmark_iters=None):
    ...
    rewards_all = []
    true_rewards_all = []
    agent_done_step_list = []
    adv_done_step_list = []
    all_done_step_list = []
    agent_info = []
    train_step = 0
    for curr_eps in range(num_runs):
        obs = env.reset()
        done = False
        adv_done = False
        agents_done = False
        ...
        while not (done or terminal):
            ...
            # print(done)
            if done:
                all_done_step_list.append(episode_step)
            adv_done = adv_done or dones[0]
            if adv_done and len(adv_done_step_list) < curr_eps + 1:
                adv_done_step_list.append(episode_step)
            agent_dones = dones[1:]
            agents_done = agents_done or any(agent_dones)
            if agents_done and len(agent_done_step_list) < curr_eps + 1:
                agent_done_step_list.append(episode_step)
        ...
        if agents_done:
            ...
            agent_done_step_list.append(args.max_episode_len)
        if adv_done:
            adv_dones_sum += 1
        else:
            adv_done_step_list.append(args.max_episode_len)
    assert len(agent_done_step_list) == num_runs
    assert len(adv_done_step_list) == num_runs
    ...
    return {'agent_sr_mean':agent_dones_sum/num_runs,'adv_sr_mean':adv_dones_sum/num_runs},  np.mean(rewards_all, axis=0), np.mean(true_rewards_all, axis=0), {"all_done_list": all_done_step_list, "agent_done_list":agent_done_step_list, "adv_done_list":adv_done_step_list}
...
def eval_msg(start_time, train_step, episode_count, agents, sr_mean, rewards):
    ...
    msg = 'time: {}, step {}, episode {}: agent success rate: {:.3f}, adv success rate: {:.3f}, return: {:.2f}, '
    ...
    print(msg.format(time_since(start_time), train_step, episode_count, sr_mean['agent_sr_mean'], sr_mean['adv_sr_mean'], cum_reward, *rewards))
...
def log_writer(args, agents):
    if args.rew_state_flag: experiment_name = "rew_stateCL_100"
    elif args.rew_action_flag: experiment_name = "rew_actionCL_100"
    elif args.state_action_flag: experiment_name = "state_actionCL_100"

    if hasattr(agents[0].env.agents[0], 'adversary'):
        run_name = 'good_%s_%s_adv_%s_noise_%.2f_%.2f_%.2f_%s_' % (experiment_name, args.good_policy, args.adv_policy, args.noise_rew, args.noise_state, args.noise_action, args.exp_folder_name)
    else:
        run_name = 'good_%s_%s_noise_%.2f_%.2f_%.2f_%s_' % (experiment_name, args.good_policy, args.noise_rew, args.noise_state, args.noise_action, args.exp_folder_name)
    run_name += str(np.datetime64('now')) if args.exp_run_num == '' else 'num_' + args.exp_run_num + '_' + str(np.datetime64('now'))
    print("Training folder name: ", run_name)
    ...
    if not os.path.isfile(success_rate_file):
        with open(success_rate_file, 'w') as f:
            ...
            f.write('episode,agent_sr_mean,adv_sr_mean,all_done_step_no,agent_done_step_no,adv_done_step_no,state_noise,reward_noise,action_noise\n')
...
def train(args):
    ...
    if args.resume:
        load_agent_states(args.resume, agents, load_models=args.use_agent_models, checkpoint=args.checkpoint)
    ...
    if args.evaluate:
        ...
        sr_mean, rewards, true_rewards, episode_end_dict = evaluate(env, agents, num_runs, args, display=args.render)
        msg = 'agent success rate: {:.3f}, adv success rate: {:.3f}, cumsum return: {:.2f}, ' + ', '.join([a.name + ': {:.2f}' for a in agents])
        print(msg.format(sr_mean['agent_sr_mean'], sr_mean['adv_sr_mean'], rewards.sum(), *rewards))
    ...
    while (episode_count <= args.max_episodes) and not terminated:
        ...
        while not (done or terminal):
            ...
            if train_step % args.eval_every == 0:
                sr_mean, rewards, _, _ = evaluate(env, agents, 50, args, display=False)
                if args.debug:
                    #writer.add_scalar('success_rate', sr_mean, train_step)
                    writer.add_scalar('success_rate', sr_mean['agent_sr_mean'], sr_mean['adv_sr_mean'], train_step)
        ...
        if episode_count % args.save_every == 0:
            ...
            sr_mean, rewards, _, episode_end_dict = evaluate(env, agents, 100, args, display=False)
            all_mean = np.mean(episode_end_dict['all_done_list'])
            all_std = np.std(episode_end_dict['all_done_list'])
            agent_mean = np.mean(episode_end_dict['agent_done_list'])
            agent_std = np.std(episode_end_dict['agent_done_list'])
            adv_mean = np.mean(episode_end_dict['adv_done_list'])
            adv_std = np.std(episode_end_dict['adv_done_list'])

            with open(files['success_rate_file'], 'a') as f:
                #line = '{},{},{},{},{}\n'.format(episode_count, sr_mean, env.noise_state, env.noise_rew, env.noise_action)
                line = '{},{},{},{}+/-{},{}+/-{},{}+/-{},{},{},{}\n'.format(episode_count, sr_mean['agent_sr_mean'],sr_mean['adv_sr_mean'], all_mean, all_std, agent_mean, agent_std, adv_mean, adv_std, env.noise_state, env.noise_rew, env.noise_action)
                f.write(line)
            ...
            # episode_diff = episode_count - start_episode
            # if sr_mean > 0.90:
            #     if flag:
            #         while(sr_mean > 0.90 and (not rew_converged)):
            #             env.noise_rew += 1.0
            #             sr_mean, rewards, _ = evaluate(env, agents, 100, args, display=False)
            #             if sr_mean <= 0.90:
            #                 env.noise_rew -= 1.0
            #                 break
            #         env.noise_state += 0.1
            #         start_episode = episode_count
            #         if not rew_converged:
            #             flag = False
            #     else:
            #         while(sr_mean > 0.90 and (not state_converged)):
            #             env.noise_state += 0.1
            #             sr_mean, rewards, _ = evaluate(env, agents, 100, args, display=False)
            #             if sr_mean <= 0.90:
            #                 env.noise_state -= 0.1
            ...
            #         env.noise_rew += 1.0
            #         start_episode = episode_count
            #         if not state_converged:
            #             flag = True
            # elif env.noise_rew == 1.0 or env.noise_state == 0.1:
            #     pass
            # else:
                # if episode_diff < 50000:
                #     pass
                # else:
                #     if flag:
                #         start_episode = episode_count
            ...
            if args.rew_state_flag:
                state_limit = 1.4 
                rew_limit = 48 
                episode_diff = episode_count - start_episode
                if sr_mean['agent_sr_mean'] > 0.90:
                    if flag:
                        # while(sr_mean['agent_sr_mean'] > 0.90 and (not rew_converged)):
                        #     env.noise_rew += 1.0
                        #     sr_mean, rewards, _ = evaluate(env, agents, 100, args, display=False)
                        #     if sr_mean <= 0.90:
                        #         env.noise_rew -= 1.0
                        ...
                        while(sr_mean['agent_sr_mean'] > 0.90 and (not state_converged)):
                            env.noise_state += 0.1
                            sr_mean, rewards, _, _ = evaluate(env, agents, 100, args, display=False)
                            if sr_mean['agent_sr_mean'] <= 0.90:
                                env.noise_state -= 0.1
                                break
                        if not rew_converged:
                            ...
                            start_episode = episode_count
                        flag = True
                        if state_converged: flag = False
                elif env.noise_rew == 1.0 or env.noise_state == 0.1:
                    pass
                else:
                    if env.noise_rew >= rew_limit: 
                        start_episode = episode_count
                        rew_converged = True
                    elif env.noise_state >= state_limit: 
                        start_episode = episode_count
                        state_converged = True
                    elif episode_diff < 100000:
                        pass
                    elif not rew_converged and not state_converged:
                        start_episode = episode_count
                        if flag:
                            env.noise_rew -= 1.0
                            rew_converged = True
                ...
                if sr_mean['agent_sr_mean'] > 0.90:
                    if flag:
                        ...
                        #         env.noise_rew -= 1.0
                        #         break
                        if not action_converged:
                            env.noise_action += 0.1
                            start_episode = episode_count
                        flag = False
                    else:
                        while(sr_mean['agent_sr_mean'] > 0.90 and (not action_converged)):
                            env.noise_action += 0.1
                            sr_mean, rewards, _, _ = evaluate(env, agents, 100, args, display=False)
                            if sr_mean['agent_sr_mean'] <= 0.90:
                                env.noise_action -= 0.1
                        ...
                        if not rew_converged:
                            ...
                            start_episode = episode_count
                        flag = True
                        if action_converged: flag = False
                elif env.noise_rew == 1.0 or env.noise_action == 0.1:
                    pass
                else:
                    if env.noise_rew >= rew_limit: 
                        start_episode = episode_count
                        rew_converged = True
                    elif env.noise_action >= action_limit: 
                        start_episode = episode_count
                        action_converged = True
                    elif episode_diff < 100000:
                        pass
                    elif not rew_converged and not action_converged:
                        start_episode = episode_count
                        if flag:
                            env.noise_rew -= 1.0
                            rew_converged = True
                ...
                if sr_mean['agent_sr_mean'] > 0.90:
                    if flag:
                        ...
                        flag = True
                        #if action_converged: flag = False
                elif env.noise_state == 0.1 or env.noise_action == 0.1:
                    pass
                else:
                    if env.noise_state >= state_limit:
                        start_episode = episode_count
                        state_converged = True
                    elif env.noise_action >= action_limit:
                        start_episode = episode_count
                        ...
                        pass
                    elif not state_converged and not action_converged:
                        start_episode = episode_count
                        if flag:
                            env.noise_state -= 0.1
                            state_converged = True
