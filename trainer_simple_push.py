...
def evaluate(env, agents, num_runs, args, display=False, max_benchmark_iters=None):
    ...
    print(np.mean(np.sum(rewards_all, axis=1)), np.std(np.sum(rewards_all, axis=1)))
    print("agent", np.mean(agent_done_step_list), np.std(agent_done_step_list))
    print("adv", np.mean(adv_done_step_list), np.std(adv_done_step_list))
    array = np.array([1]*int(agent_dones_sum)+[0]*int(num_runs-agent_dones_sum))
    print("sr", np.mean(array), np.std(array)) 
...
def log_writer(args, agents):
    ...
    if hasattr(agents[0].env.agents[0], 'adversary'):
        run_name = 'good_%s_%s_adv_%s_noise_%.2f_%.2f_%s_' % (experiment_name, args.good_policy, args.adv_policy, args.noise_rew, args.noise_state, args.exp_folder_name)
    else:
        run_name = 'good_%s_%s_noise_%.2f_%.2f_%s_' % (experiment_name, args.good_policy, args.noise_rew, args.noise_state, args.exp_folder_name)
    ...
    if not os.path.isfile(success_rate_file):
        with open(success_rate_file, 'w') as f:
            f.write('episode,agent_sr_mean,adv_sr_mean,all_done_step_no,agent_done_step_no,adv_done_step_no\n')
...
def train(args):
    ...
    if args.resume:
        ...
        load_agent_states(args.resume, agents, load_models=args.use_agent_models, checkpoint=checkpoint)
    ...
    while (episode_count <= args.max_episodes) and not terminated:
        ...
        if episode_count % args.save_every == 0:
            ...
            with open(files['success_rate_file'], 'a') as f:
                line = '{},{},{},{}+/-{},{}+/-{},{}+/-{}\n'.format(episode_count, sr_mean['agent_sr_mean'],sr_mean['adv_sr_mean'], all_mean, all_std, agent_mean, agent_std, adv_mean, adv_std)
                f.write(line)
...
def main():
    ...
    conf = 'conf:\n\tenv: %s\n\tgood vs adv: %s vs %s\n\tapprox actors: %r\n\tnoise_rew: %.2f \n\tnoise_state: %.2f' % (
        args.scenario, args.good_policy, args.adv_policy, args.use_agent_models, args.noise_rew, args.noise_state)
    print(conf)
